...
module.exports = function(dir) {
    ...
    return function(req, res, next) {
        ...
        req.app.yolog.log('verbose','adapter get data: ',res.proxyData);
        debug(res.proxyData);
        if (res.proxyData && typeof res.proxyData === 'object') {
            ...
            debug("match key:"+key);
            req.app.yolog.log('verbose','adapter match key: %s',key);
            if (adapters[key]) {
                res.proxyData = adapters[key](res.proxyData, req, res);
            }
            req.app.yolog.log('verbose','adapter after proc data:',res.proxyData);
        }
        //如果在响应上下文里面设置，不再next
        if(!res.adaptersStop) {
            next();
