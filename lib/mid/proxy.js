...
module.exports = function(req, res, next) {
  ...
  if (input.config.noApi) {
    ...
    //如果加了返回数据，则设置数据
    res.proxyData = input.config.data ? input.config.data : {};
    next();
    return;
  }
...
function procRet(params) {
  ...
  if(defaultData) {
    params.res.proxyData = _.merge(defaultData,params.res.proxyData);
  } else {
...
function callServer(params) {
  ...
  request(options, function(error, response, body) {
    if (response && response.statusCode === 200) {
      ...
      console.error('callServer:'+params.req.input.message);
      params.next();
    }
  });
}