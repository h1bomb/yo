...
function callServer(params,key) {
  ...
  params.req.app.yolog.api.log('info','server api input: ',options);
  params.req.app.yolog.profile('proxy api time:'+options.url,'api');
  request(options, function(error, response, body) {
    params.req.app.yolog.profile('proxy api time:'+options.url,'api');
    if (response && response.statusCode === 200) {
      params.req.app.yolog.api.log('info','server api status 200 output:',body);
      ...
      if (response) {
        ...
        params.req.app.yolog.api.log('error','server api error %d output: %j',response.statusCode,body,{});
      }
